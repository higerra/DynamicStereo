...
namespace dynamic_stereo {
	ProposalSegPln::ProposalSegPln(const FileIO &file_io_, shared_ptr<StereoModel<EnergyType> > model_,
	                               const Depth &noisyDisp_, const std::string &method_, const int num_proposal_)
			: Proposal(file_io_, model_), noisyDisp(noisyDisp_), method(method_),
			  num_proposal(num_proposal_) {
		params.resize(4);
		params[0] = 1;
		params[1] = 1.5;
		params[2] = 10;
	...
	void ProposalSegPln::fitDisparityToPlane(const std::vector<std::vector<int> > &seg, Depth &planarDisp, int id) {
		...
		const double dis_thres = model->nLabel * 0.01;
		printf("dis_thres: %.3f\n", dis_thres);
...
//            return (1.0 / d - min_disp) * dispResolution / (max_disp - min_disp);
		...
		for (double testdisp = 0.0; testdisp < model->nLabel; testdisp += 1.0) {
				...
				if (verbose) {
					...
					for (int i = 0; i < pts.size(); ++i)
						printf("(%.2f,%.2f,%.2f), depth: %.2f)\n", pts[i][0], pts[i][1], pts[i][2],
						       planarDisp.getDepthAtInt((int) pts[i][0], (int) pts[i][1]));
...
//		        printf("Optimal plane: (%.5f,%.5f,%.5f), %.5f\n", n[0], n[1], n[2], offset);
			...
			for (const auto idx: idxs) {
				...
				planarDisp.setDepthAtInd(idx, std::max(std::min(depthToDisp(d), (double) model->nLabel - 1), 0.0));
			}
		}

		char buffer[1024] = {};
		sprintf(buffer, "%s/temp/tdisp_%s_%03d_1.jpg", file_io.getDirectory().c_str(), method.c_str(), id);
		noisyDisp.saveImage(std::string(buffer), 255.0 / (double) model->nLabel);
		sprintf(buffer, "%s/temp/tdisp_%s_%03d_2.jpg", file_io.getDirectory().c_str(), method.c_str(), id);
		planarDisp.saveImage(std::string(buffer), 255.0 / (double) model->nLabel);
	}
	...
	ProposalSegPlnMeanshift::ProposalSegPlnMeanshift(const FileIO &file_io_,
	                                                 shared_ptr<StereoModel<EnergyType> > model_,
	                                                 const Depth &noisyDisp_,
	                                                const int num_proposal_) :
			ProposalSegPln(file_io_, model_, noisyDisp_, "meanshift",
			               num_proposal_) {
	...
	ProposalSegPlnGbSegment::ProposalSegPlnGbSegment(const FileIO &file_io_,
	                                                 shared_ptr<StereoModel<EnergyType> > model_,
	                                                 const Depth &noisyDisp_,
	                                                 const int num_proposal_) :
			ProposalSegPln(file_io_, model_, noisyDisp_, "GbSegment",
			               num_proposal_) {
		mults.resize((size_t) num_proposal);
		for (auto i = 0; i < mults.size(); ++i)
			mults[i] = (double) i + 1;
	...
	void ProposalSegPlnMeanshift::segment(const int pid, std::vector<std::vector<int> > &seg) {
		...
		CHECK_GE(pid, 0);
		const cv::Mat& image = model->image;
		segment_ms::msImageProcessor segmentor;
		segmentor.DefineImage(image.data, segment_ms::COLOR, image.rows, image.cols);
		segmentor.Segment((int) (params[0] * mults[pid]), (float) (params[1] * mults[pid]),
		                  (int) (params[2] * mults[pid]), meanshift::MED_SPEEDUP);
		...
		cv::Vec3b colorTable[] = {cv::Vec3b(255, 0, 0), cv::Vec3b(0, 255, 0), cv::Vec3b(0, 0, 255),
		                          cv::Vec3b(255, 255, 0),
		                          cv::Vec3b(255, 0, 255), cv::Vec3b(0, 255, 255), cv::Vec3b(128, 128, 0),
		                          cv::Vec3b(128, 0, 128), cv::Vec3b(0, 128, 128)};
	...
	void ProposalSegPlnGbSegment::segment(const int pid, std::vector<std::vector<int> > &seg) {
		...
		segment_gb::segment_image(model->image, output, seg,
