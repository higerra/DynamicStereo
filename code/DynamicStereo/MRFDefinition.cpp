...
//        }
    ...
    void DynamicStereo::assignDataTerm() {
	    ...
	    if (fin.is_open()) {
		    ...
		    printf("anchor:%d, resolution:%d, twindow:%d, downsample:%d, Energytype:%d\n", frame, resolution, tw, ds, type);
		    if (frame == anchor && resolution == dispResolution && tw == tWindow && type == sizeof(EnergyType) && ds == downsample) {
