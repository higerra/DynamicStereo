...
namespace dynamic_stereo {
	...
	vector<TCam> positions(N * 3, 0.0f);
	vector<TCam> axis(N * 3, 0.0f);
	vector<TCam> intrinsic;
	
	//copy data
	int index = 0;
	for(auto v=0; v<images.size(); v+=stereo_stride){
	    for(auto i=0; i<width * height * 3; ++i)
		images_data[v * width * height * 3 + i] = images[v].data[i];
	    const theia::Camera& cam = sfmModel.getCamera(v+offset);
	}
	for(auto i=0; i<width * height * 3; ++i)
