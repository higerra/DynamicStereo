...
/*******************************************************/
	...
	void msImageProcessor::FuseRegions(float sigmaS, int minRegion) {
		...
		//flag an error)
		if ((h[1] = sigmaS) <= 0) {
			ErrorHandler("msImageProcessor", "FuseRegions",
	...
	msSys.Prompt("\n 0%%");
		...
		msSys.Prompt("\r%2d%%", (int)(percent_complete + 0.5));
			...
			if ((i % PROGRESS_RATE == 0) &&
				((ErrorStatus = msSys.Progress((float) (i / (float) (L)) * (float) (0.8)))) == EL_HALT)
				break;
		}
...
/*******************************************************/
	...
	void msImageProcessor::BuildRAM(void) {
		...
		if ((!raList) &&
			((!(raList = new RAList[regionCount])) || (!(raPool = new RAList[NODE_MULTIPLE * regionCount])))) {
			ErrorHandler("msImageProcessor", "Allocate", "Not enough memory.");
			return;
		}
...
/*******************************************************/
	...
	void msImageProcessor::InitializeOutput(void) {
		...
		if ((!(modes = new float[L * (N + 2)])) || (!(labels = new int[L])) || (!(modePointCounts = new int[L])) ||
