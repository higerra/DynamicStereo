...
//#include "instances.inc"
		...
		void QPBO<REAL>::reallocate_nodes(int node_num_max_new) {
			...
			node_max[1] = nodes[1] + node_num_max;
			if (stage) {
				memmove(nodes[1], (char *) nodes[0] + ((char *) nodes_old[1] - (char *) nodes_old[0]),
				        node_num * sizeof(Node));
			}
...
/////////////////////////////////////////////////////////////////////////
		...
		bool QPBO<REAL>::Load(char *filename) {
			...
			if (K != 2) {
				printf("%s: wrong number of labels\n", filename);
				fclose(fp);
				return false;
			}
			...
			if (strcmp(LINE, type_name)) {
				printf("%s: type REAL mismatch\n", filename);
				fclose(fp);
				return false;
			}
...
#define SET_TO(a, j)             (a)->head = (j);
		...
		template<typename REAL>
		typename QPBO<REAL>::EdgeId QPBO<REAL>::AddPairwiseTerm(NodeId _i, NodeId _j, REAL E00, REAL E01, REAL E10,
		                                                        REAL E11) {
		...
		void QPBO<REAL>::AddPairwiseTerm(EdgeId e, NodeId _i, NodeId _j, REAL E00, REAL E01, REAL E10, REAL E11) {
					...
					user_assert(arcs[0][2 * e].head == &nodes[0][_i] || arcs[0][2 * e].head == &nodes[1][_i] ||
					            arcs[0][2 * e].head == &nodes[0][_j] || arcs[0][2 * e].head == &nodes[1][_j]);
					user_assert(arcs[0][2 * e + 1].head == &nodes[0][_i] || arcs[0][2 * e + 1].head == &nodes[1][_i] ||
					            arcs[0][2 * e + 1].head == &nodes[0][_j] || arcs[0][2 * e + 1].head == &nodes[1][_j]);
					user_assert(_i != _j);
			...
			if (stage == 0) {
				...
				Arc *a_rev[2] = {&arcs[0][2 * e + 1], &arcs[1][2 * e + 1]};
						code_assert(a[0]->sister == a_rev[0] && a[1]->sister == a_rev[1] && a[0] == a_rev[0]->sister &&
						            a[1] == a_rev[1]->sister);
