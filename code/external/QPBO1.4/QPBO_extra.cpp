...
#define SET_TO(a, j)             (a)->head = (j);
		...
		inline void QPBO<REAL>::AddDirectedConstraint(Node *i, Node *j, int xi, int xj) {
			...
			if (xi == 0) {
				_a[0]->r_cap = probe_options.C;
				_a[0]->sister->r_cap = 0;
			}
			else {
				_a[0]->r_cap = 0;
				_a[0]->sister->r_cap = probe_options.C;
			}
...
*/
		...
		inline bool QPBO<REAL>::AddDirectedConstraint0(Arc *a, int xi, int xj) {
			...
			if ((xi + xj + x) % 2 == 0) {
				// easy case - graph structure doesn't need to be changed
				if (a->r_cap + a->sister->r_cap + _a[1]->r_cap + _a[1]->sister->r_cap >= 2 * probe_options.C)
					return false;
				mark_node(_j[0]);
				mark_node(_j[1]);
				if (xi == 0) {
			...
			i->tr_cap += a->sister->r_cap - a->r_cap;
			_i[1]->tr_cap -= a->sister->r_cap - a->r_cap;
			a->r_cap = -a->r_cap;
		...
		struct List // contains LIST_NUM lists containing integers 0,1,...,num-1. In the beginning, each integer is in list 1.
		{
			List(int _num, int *order)
					: num(_num) {
				int i;
				prev = new int[num + LIST_NUM];
				prev += LIST_NUM;
				next = new int[num + LIST_NUM];
				next += LIST_NUM;
				if (order) {
					for (i = 0; i < num; i++) {
			...
			{
						code_assert (
						r_from >= 1 && r_from <= LIST_NUM && r_to >= 1 && r_to <= LIST_NUM && r_from != r_to);
				...
				for (i_index = list.GetNext(-current_list); i_index >= 0; i_index = i_index_next) {
					...
					for (a = i->first; a; a = a->next) {
						...
						if (IsNode0(j)) {
							...
							label[0] = GetMate1(j)->label_after_fix0;
							GetMate1(j)->label_after_fix0 = -1;
							label[1] = GetMate1(j)->label_after_fix1;
							GetMate1(j)->label_after_fix1 = -1;
						}
						for (x = 0; x < 2; x++) {
							if (label[x] >= 0 && label[1 - x] < 0) {
					...
					if (probe_options.directed_constraints != 0) {
						for (ptr = changed_list->ScanFirst(); ptr; ptr = changed_list->ScanNext()) {
							...
							if (i == j) continue;
							if (j->label_after_fix0 >= 0 && j->label_after_fix1 < 0) {
								x = 0;
								y = j->label_after_fix0;
							}
							else if (j->label_after_fix1 >= 0 && j->label_after_fix0 < 0) {
								x = 1;
								y = j->label_after_fix1;
							}
							else continue;
