...
// IT IS STRONGLY ADVISABLE TO USE INTEGERS!!! (IT IS ALSO *MUCH* FASTER).
		...
		public:
			...
			void SetMaxEdgeNum(
					int num); // If num > edge_num_max then memory for edges is reallocated. Important for Probe() with option=1,2.
			...
			void GetTwicePairwiseTerm(EdgeId e, /*output*/ NodeId &i, NodeId &j, REAL &E00, REAL &E01, REAL &E10,
			                          REAL &E11);
			...
			struct ProbeOptions {
				...
				bool (*callback_fn)(
						int unlabeled_num); // if callback_fn!=NULL, then after every testing a node Probe calls callback_fn();
		...
		private:
			...
			struct Node {
				...
				unsigned int is_sink
						: 1;    // flag showing whether the node is in the source or in the sink tree (if parent!=NULL)
			...
			Node *GetMate0(Node *i) {
						code_assert(i < nodes[1]);
				return (Node *) ((char *) i + node_shift);
			}

			Node *GetMate1(Node *i) {
						code_assert(i >= nodes[1]);
				return (Node *) ((char *) i - node_shift);
			}
			...
			Arc *GetMate0(Arc *a) {
						code_assert(a < arcs[1]);
				return (Arc *) ((char *) a + arc_shift);
			}
			...
			void ContractNodes(Node *i, Node *j,
			                   int swap); // there must hold IsNode0(i) && IsNode0(j) && (swap==0 || swap==1)
			// enforces constraint i->label = (j->label + swap) mod 2
			// i is kept, all arcs from j are deleted.
			int MergeParallelEdges(Arc *a1,
			                       Arc *a2); // there must hold (a1->sister->head == a2->sister->head) && IsNode0(a1->sister->head) &&
			...
			bool AddDirectedConstraint1(Arc *a, int xi,
			                            int xj); // ...0 checks whether submodurality needs to be swapped, ...1 preserves submodularity.
...
///////////////////////////////////////
		...
		inline typename QPBO<REAL>::NodeId QPBO<REAL>::AddNode(int num) {
			...
			if (node_last[0] + num > node_max[0]) {
				...
				if (node_num_max < (int) (node_last[0] + num - nodes[0]) + 1)
					node_num_max = (int) (node_last[0] + num - nodes[0]) + 1;
		...
		template<typename REAL>
		inline void QPBO<REAL>::GetTwicePairwiseTerm(EdgeId e, NodeId &_i, NodeId &_j, REAL &E00, REAL &E01, REAL &E10,
		                                             REAL &E11) {
			...
			if (IsNode0(j)) {
				...
				if (stage == 0) {
					E01 = 2 * a->r_cap;
					E10 = 2 * a->sister->r_cap;
				}
				else {
					E01 = a->r_cap + a_mate->r_cap;
					E10 = a->sister->r_cap + a_mate->sister->r_cap;
				}
				_j = (int) (j - nodes[0]);
			}
			else {
				...
				if (stage == 0) {
					E00 = 2 * a->r_cap;
					E11 = 2 * a->sister->r_cap;
				}
				else {
					E00 = a->r_cap + a_mate->r_cap;
					E11 = a->sister->r_cap + a_mate->sister->r_cap;
				}
				_j = (int) (j - nodes[1]);
			}
		}
