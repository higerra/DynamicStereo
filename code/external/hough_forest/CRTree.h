...
public:
	...
	CRTree(int min_s, int max_d, int cp, cv::RNG* pRNG) : min_samples(min_s), max_depth(max_d), num_leaf(0), num_cp(cp), cvRNG(pRNG) {
...
//	test[4] = cvRandInt( cvRNG ) % max_c;
	...
	test[0] = cvRNG->next() % max_w;
	test[1] = cvRNG->next() % max_h;
	test[2] = cvRNG->next() % max_w;
	test[3] = cvRNG->next() % max_h;
	test[4] = cvRNG->next() % max_c;
}
