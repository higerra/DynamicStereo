...
struct stdless
{ // wrap operator()
    bool operator()(Arg left,
		    Arg right) const{return left<right;}
};
...
namespace KDTree
{
    ...
    template < typename _Val,
	       ...
	       typename _Cmp = stdless<typename _Acc::result_type>,
	...
	KDTree( size_t nK, _Acc const& acc = _Acc(), const allocator_type& __a = allocator_type()) throw ()
	...
	template<typename _InputIterator>
        KDTree(size_t nK, _InputIterator __first, _InputIterator __last,
...
*/
	...
	find_within_range(SearchVal __V, subvalue_type const __R,
                          _OutputIterator __outIT) const throw ()        
	    {
		if (!_M_get_root()) return __outIT;
		_Region __region(mK,_M_acc,__V,__R);
		  
		return this->find_within_range(__region, __outIT);
	    }
	...
	template <typename _OutputIterator>
		...
		{
		    _Region __bounds(__REGION);
		    __outIT = _M_find_within_range(__outIT, _M_get_root(),
						   __REGION, __bounds, 0);
		}
		return __outIT;
	    }
	...
	std::pair<const_iterator,distance_type>
		...
		if (!_M_get_root())
		    return std::pair<const_iterator,distance_type>(this->end(),__Max_R);
		_Region __region(mK,_M_acc,__V);  // note: zero-area!
		typename _Region::_CenterPt __pt(__region,__Max_R);
	...
	void _M_check_children( _Link_const_type child, _Link_const_type parent, size_t const level, bool to_the_left )
		...
		if (child)
		{
		    _Node_compare compare(level % mK,_M_acc);
	...
	_M_insert(_Link_type __N, const_reference __V,
	    ...
	    {
		if (_Node_compare(__L % mK,_M_acc)(__V, __N))
		{
		    if (!_S_left(__N))
	...
	_M_get_j_min( std::pair<_Link_type,size_t> const node, size_t const level) throw ()
		...
		_Node_compare compare(node.second % mK,_M_acc);
	...
	_M_find(_Link_const_type node, const_reference value, size_t const level) const throw ()
		...
		{
		    // this line_util is the only difference between _M_find_exact() and _M_find()
	...
	_M_matches_node_in_d(_Link_const_type __N, const_reference __V,
	    ...
	    {
		_Node_compare compare(__L % mK,_M_acc);
	...
	template <typename _OutputIterator>
        _OutputIterator
        _M_find_within_range(_OutputIterator __outIT,
		...
		if (_S_left(__N))
		{
		    ...
		    __bounds.set_high_bound(_S_value(__N), __L);
		    if (__REGION.intersects_with(__bounds))
			__outIT = _M_find_within_range(__outIT, _S_left(__N),
						       __REGION, __bounds, __L+1);
		}
		if (_S_right(__N))
		{
		    ...
		    __bounds.set_low_bound(_S_value(__N), __L);
		    if (__REGION.intersects_with(__bounds))
			__outIT = _M_find_within_range(__outIT, _S_right(__N),
						       __REGION, __bounds, __L+1);
		}
	...
	_M_new_node(const_reference __V, //  = value_type(),
		...
		try
		{
		    this->_M_construct_node(__ret, __V, __PARENT, __LEFT, __RIGHT);
		}
		catch(...)
		{
	...
	*/
	...
	operator<<(std::ostream& o,
		   KDTree<_Val, _Acc, _Cmp, _Alloc> const& tree) throw ()
	    {
		o << "meta node:   " << *tree._M_header << std::endl;

		if (tree.empty())
		    return o << "[empty " << tree.mK << "d-tree " << &tree << "]";

		o << "nodes total: " << tree.size() << std::endl;
		o << "dimensions:  " << tree.mK << std::endl;

		typedef KDTree<_Val, _Acc, _Cmp, _Alloc> _Tree;
