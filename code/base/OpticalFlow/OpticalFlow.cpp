...
//--------------------------------------------------------------------------------------------------------
void OpticalFlow::getDxs(DImage &imdx, DImage &imdy, DImage &imdt, const DImage &im1, const DImage &im2, const DImage& weight)
{
	...
	CHECK_EQ(imdx.width(), weight.width());
	CHECK_EQ(imdx.height(), weight.height());

	imdx.MultiplywithAcross(weight);
	imdy.MultiplywithAcross(weight);
	imdt.MultiplywithAcross(weight);
...
//	
//--------------------------------------------------------------------------------------------------------
void OpticalFlow::SmoothFlowSOR(const DImage &Im1, const DImage &Im2, const DImage& weight, DImage &warpIm2, DImage &u, DImage &v,
                                double alpha, int nOuterFPIterations, int nInnerFPIterations, int nSORIterations)
{
	...
	for(int count=0;count<nOuterFPIterations;count++)
	{
		...
		getDxs(imdx,imdy,imdt,Im1,warpIm2, weight);
...
//	
//--------------------------------------------------------------------------------------------------------
void OpticalFlow::SmoothFlowPDE(const DImage &Im1, const DImage &Im2, const DImage& weight, DImage &warpIm2, DImage &u, DImage &v,
...
void OpticalFlow::estLaplacianNoise(const DImage& Im1,const DImage& Im2,Vector<double>& para)
{
	...
	for(int i =0;i<Im1.npixels();i++)
		...
		{
			int offset = i*nChannels+k;
			temp= std::abs(Im1.data()[offset]-Im2.data()[offset]);
...
//--------------------------------------------------------------------------------------
void OpticalFlow::Coarse2FineFlow(DImage &vx, DImage &vy, DImage &warpI2,const DImage &Im1, const DImage &Im2, const DImage& weight, double alpha, double ratio, int minWidth,
                                  int nOuterFPIterations, int nInnerFPIterations, int nCGIterations)
{
	...
	for(int k=GPyramid1.nlevels()-1;k>=0;k--)
	{
		...
		sprintf(buffer, "weight_level%d.jpg", k);
		GWeight.Image(k).imwrite(buffer);

		SmoothFlowSOR(Image1,Image2,GWeight.Image(k), WarpImage2,vx,vy,alpha,nOuterFPIterations+k,nInnerFPIterations,nCGIterations+k*3);
