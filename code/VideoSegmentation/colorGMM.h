...
//        void computeProbMat(const cv::Mat& input, cv::Mat& output) const;
        ...
        void assignComponentCuda(const std::vector<cv::Mat>& images, std::vector<cv::Mat>& output, int kChunck) const;
#endif
    private:
        ...
        std::vector<Eigen::Matrix<double, 3, 3, Eigen::ColMajor> > cov;
        std::vector<Eigen::Matrix<double, 3, 3, Eigen::ColMajor> > inverseCovs;
