...
parser.add_argument('--downsample', '-d', default=4)
...
subprocess.check_call('svm-predict {} {} tempLabel.txt'.format(args.input, args.model), shell=True)

print 'Read back and visualize...'
outputVis = np.loadtxt(open('tempLabel.txt'))
assert(outputVis is not None)

width = args.width / args.downsample
height = args.height / args.downsample
